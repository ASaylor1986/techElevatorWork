...
public class Exercises {
	...
	public int[] rotateLeft3(int[] nums) {
		...
		//int[] newInt= new int[]{first, middle, last};
		nums[0] = middle;
		nums[1] = last;
		nums[2] = first;
	...
	public int sum2(int[] nums) {
		...
		int twoOrMoreElements = nums[0] + nums[1];
		int lessThanTwo = nums[0];
		
		 if (nums.length >= 2) {
			return twoOrMoreElements; 
		 }
		 if (nums.length > 2) {
			 return lessThanTwo; 
		 }
	 ...
	 */
	public int[] middleWay(int[] aSet, int[] bSet) {
		
		int aSetMiddle = aSet[1];
		int bSetMiddle = bSet[1];
		
		return new int []{aSetMiddle, bSetMiddle};
	}
	...
	/*
	 ...
	 countEvens([1, 3, 5]) → 0
	 */
	public int countEvens(int[] evenModArray) {
		
		int counter = 0;
		for (int i = 0; i < evenModArray.length; i ++) {
			if (evenModArray[i] % 2 == 0) {
				counter ++;
			}
		}
		return counter;
	...
	/*
	 ...
	 sum13([1, 2, 2, 1, 13]) → 6
	 */
	public int sum13(int[] sumOfElements) {
	
		int sum = 0;
		
		for (int i = 0; i < sumOfElements.length; i ++) {
			if (sumOfElements[i] != 13) {
				sum = sum + sumOfElements[i];
			}
			else {
				i++;
			}
	...
	/*
	 ...
	 has22([2, 1, 2]) → false
	 */
	public boolean has22(int[] pairOrNot) {
		boolean pairOfElements= true;
		
		for (int i = 0; i < pairOrNot.length -1; i ++) {
			if ((pairOrNot[i] == 2) && (pairOrNot[i + 1] == 2)) {
	...
	/*
	 ...
	 lucky13([1, 2, 4]) → false
	 */
	public boolean lucky13(int[] luckyNums) {
		boolean noOnesOrThrees = true;
		
		for (int i = 0; i < luckyNums.length; i ++) {
			if ((luckyNums[i] == 1) || (luckyNums[i] == 3)) {
				return false;
			}
		}
		return noOnesOrThrees;
	}
	...
	/*
	 ...
	 sum28([1, 2, 3, 4]) → false
	 */
	public boolean sum28(int[] crazyEights) {
		// I worked on this for 3 hours, and still can't figure it out...
		boolean isCrazy = true;
		int counter = 0;
		
		for (int i = 0; i < crazyEights.length; i ++) {
			if (crazyEights[i] == 2) {
				counter ++;
				}
		}
