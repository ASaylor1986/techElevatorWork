...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		List < String > listOfString = Arrays.asList(stringArray);
		return listOfString;
	}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for( int i = 0; i < stringArray.length; i++) {
			if (stringArray[i].length() != 4) {
				notFourLetters.add(stringArray[i]);
			}
		}
	...
	public List<String> reverseList(List<String> stringList) {
		...
		List<String> noIdeaWhatImDoing = new ArrayList<>();
		for ( int i = 0; i < stringList.size(); i ++) {
			noIdeaWhatImDoing.add(0, stringList.get(i));	
		}
		return noIdeaWhatImDoing;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List<Double> goneCrazy = new ArrayList<Double>();
		
		for (int i = 0; i < intArray.length; i ++) {
		double answer = (double) intArray[i] / 2;
		goneCrazy.add(answer);
		}
		return goneCrazy;
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		List<Integer> onlyOddNums = new ArrayList<Integer>();
		
		for( int i = 0; i < integerArray.length; i++) {
			if (integerArray[i] % 2 == 1) {
				onlyOddNums.add(integerArray[i]);
			}
		}
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		List<String> mtDew = new ArrayList<String>();
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> basketWeaving = new ArrayList<Integer>();
		
		if (listOne.size() >= listTwo.size()) {
			for( int i = 0; i < listOne.size(); i++) {
				if ( i < listTwo.size()) {
					basketWeaving.add(listOne.get(i));
					basketWeaving.add(listTwo.get(i));
				}
				else {
					basketWeaving.add(listOne.get(i));
				}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		List<Integer> seatAssignment = new LinkedList<Integer>();
		Queue<Integer> queueOne = new LinkedList<Integer>();
		Queue<Integer> queueTwo = new LinkedList<Integer>();
		Queue<Integer> queueThree = new LinkedList<Integer>();
		
		for (int seatNumber: seatNumberList) {
			if (seatNumber>=1 && seatNumber<=10) {
				queueOne.add(seatNumber);
			}
			if (seatNumber>=11 && seatNumber<=20) {
				queueTwo.add(seatNumber);
			}
			if (seatNumber>=21 && seatNumber<=30) {
				queueThree.add(seatNumber);
			}
		}
		
		seatAssignment.addAll(queueOne);
		seatAssignment.addAll(queueTwo);
		seatAssignment.addAll(queueThree);
