...
-- Countries sorted by form of govt., region, gnp

SELECT name, governmentform, region, gnp FROM country
ORDER BY governmentform, region, gnp;
...
SELECT SUM(population) AS total_pop, (AVG(population), 2) AS average_pop, COUNT(*) FROM city;
...
SELECT continent, AVG(lifeexpectancy) AS avg_lifespan 
FROM country 
GROUP BY continent 
...
SELECT continent, round(AVG(lifeexpectancy):: numeric, 2) AS avg_lifespan 
FROM country 
WHERE lifeexpectancy is not null
GROUP BY continent 
ORDER BY avg_lifespan DESC;
...
SELECT c.name, co.name FROM city AS c, country AS co WHERE countrycode = 'ABW';
