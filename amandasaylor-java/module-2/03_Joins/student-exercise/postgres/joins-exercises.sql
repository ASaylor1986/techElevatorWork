...
-- (3 rows)
SELECT title, rating, length, cat.name
FROM film AS f
JOIN film_category AS fc ON fc.film_id = f.film_id
JOIN category AS cat ON cat.category_id = fc.category_id
...
SELECT film.title, rating, actor.first_name, actor.last_name
FROM film 
JOIN film_actor ON film_actor.film_id = film.film_id
JOIN actor ON actor.actor_id = film_actor.actor_id
...
SELECT title, release_year, cat.name
FROM film AS f
JOIN film_category AS fc ON fc.film_id = f.film_id
JOIN category AS cat ON cat.category_id = fc.category_id
...
SELECT store.store_id, address, staff.first_name AS manager_first_name, staff.last_name AS manager_last_name
FROM address
JOIN store ON address.address_id = store.address_id
