...
<c:if test="${errorMessage != null}">
	<div style="color:hotPink">${errorMessage}</div>
</c:if>
...
<form method="GET" action="${formAction}">
	...
	<div class="formInputGroup">
		...
		<select  name="loanTerm" id="loanTerm">
			<option value="10" ${loanTerm == 10? "selected": ""} >10 Years</option>
			<option value="15" ${loanTerm == 15? "selected": ""}>15 Years</option>
			<option value="30" ${loanTerm == 30? "selected": ""}>30 Years</option>
		</select>
	</div>
